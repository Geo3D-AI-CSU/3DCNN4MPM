...
def get_model(inputs, is_training, bn_decay = None):     #模型参数
	...
	print("反卷积和特征通道总训练时长是：",time.time()-deco_t)
	return layer


def get_bigmodel(inputs, is_training, bn_decay=None):  # 模型参数
...
考虑两个类别
'''
# def dice_loss(output, target, loss_type='jaccard', axis = (0,1), smooth=1e-5): 徐章皓修改
def dice_loss(output, target, loss_type='jaccard', axis = (1,2,3,4), smooth=1e-5):
	...
	target = tf.cast(tf.one_hot(indices = target, depth = 2, on_value = 1, off_value = 0),dtype = tf.float32)
	# 徐章皓修改
...
def focal_loss(pred, label, alpha = 0.25, gamma = 2):
	...
	y_true = tf.one_hot(label, 2) #二分类
	probs = tf.nn.sigmoid(pred) #用的是sigmoid
	...
	# focal_loss = tf.reduce_mean(focal_xent)
