...
def change_format(data, batchsize, deep, height, weight, channel, mark):
	"""
	   ...
	   - weight: 数据宽度
	   - channel: 数据通道数
	   - mark: 'TRUE' 表示转换为多维数组，'FALSE' 表示转换为多维张量
...
def read_csv(filepath, filenames, batchsize, deep, height, weight, channel):
	"""
	    ...
	    batchsize : int
	        每个batch的样本数量。
	    deep : int
	        数据的深度（在三维数据中通常表示Z轴的大小）。
	    height : int
	        数据的高度（在三维数据中通常表示Y轴的大小）。
	    weight : int
	        数据的宽度（在三维数据中通常表示X轴的大小）。
	    channel : int
	        数据的通道数。
	...
	for name in filenames:
		...
		csvfile = open(os.path.join(filepath, name), 'r') 							  # 打开CSV文件
		reader = csv.reader(csvfile)												  # 使用CSV读取器读取文件内容
		for item in reader:															  # 遍历CSV文件的每一行
			if reader.line_num == 1:
				continue															  # 跳过第一行，因为它通常包含属性名
			# data.append(item[3:-2]) #前三列为I,J,K; 后两列为标签
			# [3:-2]是指从第4列到倒数第三列
			a = np.array(item[3:10])
			# b = np.array(item[16:20])
			# data.append(np.concatenate((a,b)).tolist())
			data.append(a.tolist()) 												  # 将数据转换为列表并添加到data列表中
			label.append(item[-1])  												  # 后两列为标签，读取最后一列为非稀疏
	# 将数据和标签转换为NumPy数组
	data = np.array(data)
	label = np.array(label)
...
def cross_validation(filelist, fold):
 	'''
    Input:
    filelist: 文件名列表，以数组形式提供
    fold: 折数，即交叉验证的折数

    Output:
    train_list: 包含每个类别训练集文件名的列表
    test_list: 包含每个类别测试集文件名的列表

    使用KFold方法进行交叉验证，将数据按照指定折数划分为训练集和测试集。
	'''
...
def placeholder_inputs(batch_size, size, channel):
	"""
	   创建用于输入数据的 TensorFlow 占位符。

	   Parameters:
	   - batch_size: 批次大小
	   - size: 数据的尺寸（在三个维度上的大小相同，例如立方体数据）
	   - channel: 数据通道数

	   Returns:
	   - pointclouds_pl: 输入点云数据的占位符，形状为 (batch_size, size, size, size, channel)
	   - labels_pl: 输入标签数据的占位符，形状为 (batch_size, size, size, size)
	   """
	# 创建用于输入点云数据的占位符，数据类型为 float32
    pointclouds_pl = tf.placeholder(tf.float32,
                                     shape=(batch_size, size, size, size, channel))
	# 创建用于输入标签数据的占位符，数据类型为 int32，非稀疏表示
...
def log_string(LOG_FOUT, out_str):
	"""
	    ...
	    Parameters:
	    - LOG_FOUT: 文件句柄，表示日志文件
	    - out_str: 要输出的字符串
	    """
    LOG_FOUT.write(out_str+'\n')												 # 将字符串写入到日志文件，末尾添加换行符
    LOG_FOUT.flush()															 # 立即刷新日志文件，确保写入内容即时生效
    print(out_str)  															 # 在控制台打印相同的字符串
...
def get_learning_rate(base_learning_rate, 
	...
	"""
        ...
        Returns:
        - learning_rate: 调整后的学习率
        """
	# 使用指数衰减函数进行学习率的动态调整
    learning_rate = tf.train.exponential_decay(
                        base_learning_rate,  							# 初始学习率
                        batch * batch_size,  							# 当前迭代步数（批次乘以批次大小）
                        decay_step,          							# 学习率衰减步数
                        decay_rate,         							# 学习率衰减率
                        staircase=True)	 								# 是否使用阶梯式（Staircase）衰减
    learning_rate = tf.maximum(learning_rate, 0.00001) 					# 将学习率限制在一个最小值（0.00001）以上，避免学习率过小
...
"""
    ...
    Parameters:
    - bn_init_decay: 初始批归一化衰减系数
    - batch: 当前批次数
    - batch_size: 批次大小
    - bn_decay_decay_step: 批归一化衰减步数
    - bn_decay_decay_rate: 批归一化衰减率
    - bn_decay_clip: 批归一化衰减系数的上限值

    Returns:
    - bn_decay: 调整后的批归一化衰减系数
    """
# 使用指数衰减函数进行批归一化衰减系数的动态调整
def get_bn_decay(bn_init_decay,											# 初始批归一化衰减系数
                 batch, batch_size, 									# 当前迭代步数（批次乘以批次大小）
                 bn_decay_decay_step,									# 批归一化衰减步数
                 bn_decay_decay_rate, 									# 批归一化衰减率
                 bn_decay_clip):
    bn_momentum = tf.train.exponential_decay(
                      ...
                      bn_decay_decay_rate,
                      staircase=True)
    bn_decay = tf.minimum(bn_decay_clip, 1 - bn_momentum)				# 将批归一化衰减系数限制在一个最大值（bn_decay_clip）以下
...
def conv3D(inputs, 
	...
	"""
	   ...
	   参数：
	   - inputs: 输入张量。
	   - kernel_size: 卷积核大小，格式为 [depth, height, width]。
	   - input_channels: 输入通道数。
	   - output_channels: 输出通道数。
	   - strides: 步幅，格式为 [depth_stride, height_stride, width_stride]。
	   ...
	   返回：
	   - result: 经过卷积、激活、可选的 dropout 和批归一化处理后的张量。
	   """
...
def max_pooling_3D(inputs, 
	...
	"""
	    ...
	    参数：
	    - inputs: 输入张量。
	    - kernel_size: 池化核大小，格式为 [depth, height, width]。
	    - strides: 步幅，格式为 [depth_stride, height_stride, width_stride]。
...
def dice_loss(output, target, loss_type='jaccard', axis=(1,2,3,4), smooth=1e-5):
	"""
    计算 Dice Loss，结合了 Sorensen 或 Jaccard 系数，用于比较两个数据批次的相似性，通常用于二进制图像分割任务。

    参数：
    - output: 网络输出的预测张量。
    - target: 真实标签张量。
    - loss_type: 损失类型，可选 'jaccard' 或 'sorensen'，默认为 'jaccard'。
    - axis: 指定计算 Dice Loss 时进行计算的轴，默认为 (1,2,3,4)。
    - smooth: 平滑项，防止分母为零，默认为 1e-5。
...
def focal_loss(pred, label, alpha = 0.25, gamma = 2):
	"""
	    计算 Focal Loss，用于处理类别不平衡问题，特别适用于二分类任务。

	    参数：
	    - pred: 网络输出的预测张量。
	    - label: 真实标签张量。
	    - alpha: 平衡因子，调整正负样本权重，默认为 0.25。
	    - gamma: Focal Loss 的调制因子，默认为 2。

	    返回：
	    - focal_loss: Focal Loss 的值。
	    """
...
def get_loss(pred, label):
	"""
	  计算损失函数，结合了 Dice Loss 和 Focal Loss。
	  ...
	  返回：
	  - hybird_loss: 结合了 Dice Loss 和 Focal Loss 的损失值。
	  """
